...
export function eventHandler<T>(target: any, propertyKey: string) {
  const privateKey = Symbol(`__${propertyKey}`);

  Object.defineProperty(target, propertyKey, {
    get() {
      if (!this[privateKey]) {
        // Toma la función original de la instancia
        const original = Object.getOwnPropertyDescriptor(this, propertyKey)?.value;
        if (typeof original !== 'function') {
          throw new Error(`@eventHandler solo puede aplicarse a propiedades función.`);
        }
        const handler: EventHandler<T> = Object.assign(
          (payload: T) => original.call(this, payload),
          {
            id: Symbol(propertyKey),
            className: this.constructor.name,
            tagName: typeof this.tagName === 'string' ? this.tagName : undefined,
          }
        );
